...
def export(*a):
    ...
    if path == None or path == '':
        path = pm.fileDialog2(fm=3, dir=cfb.LK_ASSET_DIR)[0]
        path += '\\' + name + '\\'
...
def importAsset( get_file=None, *a):
    """ Opens an import dialog starting at the CFB asset directory, 
        using no namespaces"""
    if not get_file:
        get_file = pm.fileDialog2(dir=cfb.LK_ASSET_DIR, ds=1, fm=1)
...
def getAssetList( typ='generic' ):
    """ Return a list of all assets (folders) in various asset directories, 
        defined in 'typ' """
    if typ == 'generic':
        asset_list = os.listdir(cfb.LK_ASSET_DIR)
